...
interface Task {
  id: string
  taskType: 'insurance' | 'payment' | 'documents'
  taskName: string
  deadlineDate: string
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="border-b border-white/10 bg-slate-900/60">
                        ...
                        <th className="py-4 px-6 text-xs font-bold uppercase tracking-wider text-slate-400 w-[55%]">Завдання та дедлайни</th>
                      </tr>
                    </thead>
                        ...
                        <tr key={tour.id} className="hover:bg-white/[0.02] transition-colors">
                          ...
                          <td className="py-5 px-6">
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                              {['insurance', 'payment', 'documents'].map(type => {
                                const task = tour.tasks.find(t => t.taskType === type)
                                if (!task) return null
                                  ...
                                  >
                                    <div className="flex items-center justify-between w-full">
                                      <span className="text-[11px] font-extrabold uppercase tracking-wide opacity-80">
                                        {type === 'insurance' ? 'Страховка' : type === 'payment' ? 'Доплата' : 'Документи'}
                                      </span>
                                      {isUpdating ? (
